...
### Task 6: apply() — full turn resolution and scoring ✅ DONE (quality review skipped by user; spec review incl. adversarial probes passed) (4e186ae; test note: a 2-tile shielded city is unbuildable with real tiles — majority test uses the smallest real shielded city, E+F+E = 3 tiles + shield = 8 pts)
...
### Task 19: Learner + resume ✅ DONE (efa5630; D4 = rotations only, resampled through the engine for correctness-by-construction — reflections excluded as base-game tiles are chiral; NaN-safe policy loss via torch.where over the target support; metrics keys per caller contract {step, loss, policy_loss, value_loss, buffer_games, buffer_examples})
**Depends on:** Task 18
...
### Task 20: Arena gating + `train` CLI orchestration ✅ DONE (9d1b806)
**Depends on:** Task 19
